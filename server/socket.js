...
/////////// Check Online or OFfline Users //////////
    ...
    socketConnection(){
        ...
        this.io.on('connection',(socket)=>{
            ...
            this.checkViewOrRevived(socket);
    ...
    checkViewOrRevived(socket){

        socket.on('checkViewOrRevived',(data)=>{
            this.emit(data.to,'setViewOrRevived',{from:socket.id,toId:data.toId,to:data.to,massages:data.messages_id});


            // this.emit(data.toId,'setViewOrRevived',{massages:data.messages_id});
            // this.emit(socket.id,'setViewOrRevived',{massages:data.messages_id});
          //  this.emit(socket.id,'setViewOrRevived',{user_id_open:data.user_id_open});
    ...
    private_message(socket){
        ...
        socket.on('send_private_msg',(data)=>{
            this.emit(socket.id,'new_private_msg',{username:socket.username,from_uid:data.to,whois:socket.id,message:data.message,toId:data.toId,image:data.image,massage_id:data.massage_id,created_at:data.created_at});

            this.emit(data.to,'new_private_msg',{username:socket.username,from_uid:socket.id,whois:socket.id,message:data.message,toId:data.toId,image:data.image,massage_id:data.massage_id,created_at:data.created_at});
