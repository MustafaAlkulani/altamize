                ...
                <div class="row" id="page-contents">
                    ...
                    <div class="col-md-8 ">
                        ...
                        <div class="central-meta">
                            <div class="messages">
                                <h5 class="f-title"><i class="ti-bell"></i>{{trans('social.All Messages')}} <span class="more-options"><i
                    ...
                    <div class="col-md-4">
                        <aside class="sidebar static">
                            ...
                            <div class="widget friend-list stick-widget">
                                <h4 class="widget-title">{{trans('social.Friends')}}</h4>
                                ...
                                <ul id="people-list" class="friendz-list row">
                                    ...
                                    @foreach($users as $user)
                                        <div class="more col-xs-12">
                                            <li id="sidebar-user-box" class="user" uid="{{ $user->id }}">
                                                <figure>
                                                    <img class="user_image"
                                                         src="{{Storage::url($user->personal_image)}}"
                                                         style="width:100px " alt="">
                                                    {{--<span class="status  user_status user_{{ $user->id }} f-online"></span>--}}

                                                    <span id="user_status_user_{{ $user->id }}"
                                                          class="user_status user_{{ $user->id }}"></span>
                                                </figure>
                                                <div class="friendz-meta">
                                                    <a href="{{surl('personalPage/'.$user->id)}}">{{$user->display_name}}</a>
                                            ...
                                            <ul class="more-dropdown  UserMassager-dropdown">
                                                <div class="setting-row">
                                                    <span>  <a href="{{surl('personalPage/'.$user->id)}}">{{trans('social.View Profile')}}</a></span>

                                                </div>

                                                <div class="setting-row">
                                                    <span>{{trans('social.Blook')}}</span>
                                                    <?php
                                                    $isBlocked = false;
                                                    ?>
                                                    {{--<button type="button" " style="display: none" ></button>--}}

                                                    <input blocked="@if(canNotMassageMe($user->id,social()->user()->id)==false) 1 @else 0 @endif"
                                                           class="@if(isBlokedUser(social()->user()->id,$user->id) )blocked<?php
                                                           $isBlocked = true;
                                                           ?>@ELSE unblocked<?php $isBlocked = false;?>@ENDIF blockBtn"
                                                           type="checkbox" id="switch{{$user->id}}"
                                                           @if($isBlocked)
                                                           checked
                                                           @ENDIF
                                                           uid="{{ $user->id }}"/>
                                                    <label for="switch{{$user->id}}" data-on-label="ON"
                                                ...
                                                <div class="setting-row">
                                                    <span>  <a class="buttonDeleteConversation" url="deleteAllMessages"
                                                               ...
                                                               user-id="{{$user->id}}">{{trans('social.Delete Conversation')}}
...
@section('footer')
    ...
    {{--<script src="{{url('/')}}/Desing/social/LowgaChat-design/js/jquery-1.10.1.min.js"></script>--}}
    <!-- jQuery 2.1.4 -->
...
//
           ...
           */
            ...
            var mass_name = '{{$user_messages->display_name}}';
            var mass_id = '{{$user_messages->id}}';
            var mass_image = '{{$user_messages->personal_image}}';
    ...
    <script src="{{url('/')}}/Desing/social/js/socket.io.js"></script>
