...
//  dd($result);
    ...
    public function store(Request $request ,$type)
    {
        ...
        // dd($study_plan_id);
        $GLOBALS['study'] = Study_plan::find($request->study_plan_id);
...
//                    $GLOBALS['numRow'] = intval($request->get('numRow'));
...
//                    $GLOBALS['numStudent'] = $request->get('numStudent');
                    ...
                    Excel::load($path, function ($reader) 
                    {
                        ...
                        if (count($GLOBALS['id']) > 0) {
                         ...
                         {
                        $title = ' no  ';
                        session()->flash('error', trans('لا يوجد طالب   '));
                        return back()->with('title',$title );
                            ...
                            if (empty($error_data)) {
                                if (!empty($insert_data)) {
                                    ...
                                    if($type=="add")
                                    session()->flash('success', trans('تم الاضافة بنجاح'));
                                    else
...
//                       else {     $title = ' عرض الخطا  ';
//                          session()->flash('error', trans('المادة غير موجودة لهذا القسم     '));
//                        return back()->with('title',$title );}
//
...
//dd($study_plan_id);
        ...
        if ($grades <= $study->max_grade && $grades > -1) {
         ...
         session()->flash('error', trans('الدرجة التي ادخلتها غير صحيحة'));
        return  redirect()->back();
    }
    ...
    public function showResult()
    {
        ...
        return view('admin.control.showResult', compact( 'title'));

    }



    public function showResult2(Request   $request)
    {
        ...
        $id = '';
        $student = Student::where('acadimy_id',$request->acadimy_id )->first();
        if($student){
        ...
        $title = "نتيجتي";
        return view('admin.control.showResult', compact('results', 'title','student','dept'));
         }else {    $title = "نتيجتي";
            session()->flash('error', trans('لا يوجد طالب بهدا الرقم'));
            return view('admin.control.showResult', compact( 'title'));}
    }
