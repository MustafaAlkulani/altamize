...
class SocailDepartmentController extends Controller
{
    ...
    public function Infolevel($id, $level, LevelDepartmentDatatable $student)
    {
        ...
        $dataStudy = Study_plan::where(['department_id' => $id, 'level' => $level, 'semester' => currentSemester()])->get();
        ...
        return $student->render('admin.social.section.level.index', compact('data', 'title', 'level', 'countStudent', 'countStudy', 'countGroup','dataStudy'));
    }
...
//            GroupMember::where(['group_id'=>$group_id,'id'=>$member_id])->delete();
    ...
    public function StudyCourseCreate($id){
        $data=Study_plan::findOrFail($id);
        $title = 'اضافة كورس للمادة : '.$data->name_ar;
        return view('admin.social.courses.add', compact('title', 'data'));
    }

    public function studyCourseCreateAdd($id,Request $request){
        $dataS=Study_plan::findOrFail($id);
        ...
        $data['study_plan_id'] =$id;
        $data['department_id'] =$dataS->department_id;
...
//        dd($data['year']);
        ...
        if (empty($newStudyCourse)) {
           $n= StudyCourse::updateOrCreate(['teacher_id' => $data['teacher_id'], 'study_plan_id' => $data['study_plan_id'], 'year' => $data['year']]);
            session()->flash('success', 'تم اضافة كورس جديد بنجاح');
            return redirect(asurl('/'.$n->id.'/course'));
        } else {
            session()->flash('success', 'هذا الكورس مضاف مسبقا ');
            return redirect(asurl('/'.$newStudyCourse->id.'/course'));
        }
    }
...
//        dd($data['year']);
    ...
    {
        $groups= new AddUserGroupDatatable(1,$id);
        $dataCourse = StudyCourse::findOrFail($id);
...
//       $d=Study_plan::find($data->study_plan_id)->department_id;
    ...
    public function groupAdd($id, Request $request)
    {
        ...
        $group_id = $group_->id;
        if ($dataCourse->teacher_id) {

            $teacher = $dataCourse->teacher_id;
...
//
    ...
    public function countfetch(Request $request){
        ...
        if ($request->ajax()) {
            ...
            $user=UserAccount::find(2);
//            $date= $date.$date;
//           return ($user->created_at);
           $user=UserAccount::where('created_at','>',$date)->count();
            $post=Post::where('created_at','>',$date)->count();
            $group=Group::where('created_at','>',$date)->count();
            $reference=ReferenceBook::where('created_at','>',$date)->count();
            $study=StudyCourse::where('created_at','>',$date)->count();
            $notify=Notification::where('created_at','>',$date)->count();
            $upgrade=Upgrade::where('created_at','>',$date)->count();


            $output = ['user'=>$user,'post'=>$post,'group'=>$group,'reference'=>$reference,'notify'=>$notify,'upgrade'=>$upgrade,'study'=>$study];
    ...
    public function isBlockedGroup($group_id,$user_id){
        $data=GroupMember::where(['group_id'=>$group_id,'user_id'=>$user_id])->first();
        if($data ){
            if($data->isBlocked == 0){
                GroupMember::where(['group_id'=>$group_id,'user_id'=>$user_id])->update(['isBlocked'=>1]);
            }
            else{
                GroupMember::where(['group_id'=>$group_id,'user_id'=>$user_id])->update(['isBlocked'=>0]);

            }
        }

        session()->flash('success', 'تم تحديث حالة العضو في المجموعة بنجاح');
        return redirect()->back();
    }

    public function isAdminGroup($group_id,$user_id){
        $data=GroupMember::where(['group_id'=>$group_id,'user_id'=>$user_id])->first();
        if($data ){
            if($data->isAdmin == 0){
                GroupMember::where(['group_id'=>$group_id,'user_id'=>$user_id])->update(['isAdmin'=>1]);
            }
            else{
                GroupMember::where(['group_id'=>$group_id,'user_id'=>$user_id])->update(['isAdmin'=>0]);
